...
Suites.enable = function (names, tags) {
  if (names?.length) {
    ...
    this.forEach((suite) => {
      ...
      else suite.disabled = true;
    });
  } else if (tags?.length) {
    tags.forEach((tag) => {
      if (!Tags.has(tag)) console.error(`Unknown Suites tag: "${tag}"`);
    });
    ...
    console.warn(
      "Neither names nor tags provided. Enabling all default suites."
    );
    this.forEach((suite) => {
      suite.disabled = !("default" in suite.tags);
    });
  }
  if (this.some((suite) => !suite.disabled)) return;
...
Suites.push({
  ...
  tests: [
    new BenchmarkTestStep(`Adding${numberOfItemsToAdd}Items`, (page) => {
      const input = page.querySelector(".new-todo-input", [
        "todo-app",
        "todo-topbar",
      ]);
      for (let i = 0; i < numberOfItemsToAdd; i++) {
...
Suites.forEach((suite) => {
  ...
  else if (suite.disabled) suite.tags.unshift("all");
  else suite.tags.unshift("all", "default");
...
export const Tags = new Set([
  ...
  "default",
  "tentative",
  ...Suites.flatMap((suite) => suite.tags),
]);
Object.freeze(Tags);
